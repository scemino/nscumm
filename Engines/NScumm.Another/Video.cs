...
namespace NScumm.Another
{
    internal class Video
    {
        ...
        private static readonly Dictionary<int,string> StringsTableFr = new Dictionary<int, string>
        {
            { 0x001, "P E A N U T  3000" },
            { 0x002, "Copyright  } 1990 Peanut Computer, Inc.\nAll rights reserved.\n\nCDOS Version 5.01" },
            { 0x003, "2" },
        ...
        private Dictionary<int, string> _stringsTable;
        ...
        public Video(Resource res, IAnotherSystem system)
        {
            _res = res;
            _sys = system;
            _stringsTable = Engine.Instance.Settings.Game.Language == Language.FR_FRA
                ? StringsTableFr
                : StringsTableEng;
        }
	...
	dumpFrameBuffers(name);
        ...
        {
            //Not found
            if (!_stringsTable.ContainsKey(stringId))
                return;

            //Search for the location where the string is located.
            var se = _stringsTable[stringId];
