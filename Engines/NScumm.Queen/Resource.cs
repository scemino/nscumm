...
namespace NScumm.Queen
{
	...
	public partial class Resource : IDisposable
	{
		...
		public Resource()
		{
			...
			Stream stream;
			if ((stream = Engine.OpenFileRead("queen.1c")) == null)
			{
				if ((stream = Engine.OpenFileRead("queen.1")) == null)
				{
					D.Error("Could not open resource file 'queen.1[c]'");
				}
			}
			_resourceFile = new BinaryReader(stream);
		...
		private void SeekResourceFile(int num, uint offset)
		{
			if (_currentResourceFileNum != num)
			{
				...
				var stream = Engine.OpenFileRead(name);
				_resourceFile = new BinaryReader(stream);
		...
		{
			var tableStream = Engine.OpenFileRead(TableFilename);
			if (tableStream != null)
			{
				using (var tableFile = new BinaryReader(tableStream))
				{
		...
		public static DetectedGameVersion DetectVersion(Stream file)
		{
			...
			else
			{
				...
				if (Equals(gameVersion, _gameVersions[VER_AMI_DEMO]))
				{ // CE101
				...
				if (Equals(gameVersion, _gameVersions[VER_AMI_INTERVIEW]))
				{ // PE100
					ver.language = Language.EN_ANY;
