    ...
    internal partial class Kernel
    {
        ...
        private static Register kDoBresen(EngineState s, int argc, StackPtr argv)
        {
            ...
            if (ResourceManager.GetSciVersion() >= SciVersion.V1_EGA_ONLY)
            {
                ...
                SciEngine.WriteSelectorValue(segMan, client, o => o.signal,
                    (ushort) (client_signal & ~ViewSignals.HitObstacle));
            }
        ...
        private static Register kDoAvoider(EngineState s, int argc, StackPtr argv)
        {
            SegManager segMan = s._segMan;
            Register avoider = argv[0];
            short timesStep = (short) (argc > 1 ? argv[1].ToUInt16() : 1);

            if (!s._segMan.IsHeapObject(avoider))
            {
                Error("DoAvoider() where avoider {0} is not an object", avoider);
                return Register.SIGNAL_REG;
            }

            Register client = SciEngine.ReadSelector(segMan, avoider, o => o.client);
            Register mover = SciEngine.ReadSelector(segMan, client, o => o.mover);
            if (mover.IsNull)
                return Register.SIGNAL_REG;

            // call mover::doit
            SciEngine.InvokeSelector(s, mover, o => o.doit, argc, argv);

            // Read mover again
            mover = SciEngine.ReadSelector(segMan, client, o => o.mover);
            if (mover.IsNull)
                return Register.SIGNAL_REG;

            short clientX = (short) SciEngine.ReadSelectorValue(segMan, client, o => o.x);
            short clientY = (short) SciEngine.ReadSelectorValue(segMan, client, o => o.y);
            short moverX = (short) SciEngine.ReadSelectorValue(segMan, mover, o => o.x);
            short moverY = (short) SciEngine.ReadSelectorValue(segMan, mover, o => o.y);
            short avoiderHeading = (short) SciEngine.ReadSelectorValue(segMan, avoider, o => o.heading);

            // call client::isBlocked
            SciEngine.InvokeSelector(s, client, o => o.isBlocked, argc, argv);
            ...
            if (s.r_acc.IsNull)
            {
                ...
                ushort angle = kGetAngleWorker(clientX, clientY, moverX, moverY);

                Register clientLooper = SciEngine.ReadSelector(segMan, client, o => o.looper);
                ...
                {
                    // call looper::doit
                    Register[] @params = {Register.Make(0, angle), client};
                    SciEngine.InvokeSelector(s, clientLooper, o => o.doit, argc, argv, 2, new StackPtr(@params));
                }
                s.r_acc = Register.SIGNAL_REG;
            }
            else
            {
                // is blocked
                if (avoiderHeading == -1)
                    avoiderHeading = (short) (SciEngine.Instance.Rng.GetRandomBit() != 0 ? 45 : -45);
                short clientHeading = (short) SciEngine.ReadSelectorValue(segMan, client, o => o.heading);
                clientHeading = (short) ((clientHeading / 45) * 45);

                short clientXstep = (short) (SciEngine.ReadSelectorValue(segMan, client, o => o.xStep) * timesStep);
                short clientYstep = (short) (SciEngine.ReadSelectorValue(segMan, client, o => o.yStep) * timesStep);
                short newHeading = clientHeading;
                ...
                while (true)
                {
                    ...
                    SciEngine.WriteSelectorValue(segMan, client, o => o.x, (ushort) newX);
                    SciEngine.WriteSelectorValue(segMan, client, o => o.y, (ushort) newY);
                    ...
                    if (!s.r_acc.IsNull)
                    {
                        s.r_acc = Register.Make(0, (ushort) newHeading);
                        break; // break out
                    }
                    ...
                    {
                        // tried everything
                        SciEngine.WriteSelectorValue(segMan, client, o => o.x, (ushort) clientX);
                        SciEngine.WriteSelectorValue(segMan, client, o => o.y, (ushort) clientY);
                        s.r_acc = Register.SIGNAL_REG;
                        break; // break out
                    }
                }
            }
            SciEngine.WriteSelectorValue(segMan, avoider, o => o.heading, (ushort) avoiderHeading);
            return s.r_acc;
        }
    }
}