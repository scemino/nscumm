...
namespace NScumm.Sci.Engine
{
        ...
        public override bool FindRecord(MessageTuple tuple, out MessageRecord record)
        {
            ...
            for (uint i = 0; i < _messageCount; i++)
            {
                if ((recordPtr[0] == tuple.noun) && (recordPtr[1] == tuple.verb))
                {
                    ...
                    record.talker = 0;
                    record.@string = ScummHelper.GetText(_data, recordPtr.ReadUInt16(2));
                    return true;
                }
        ...
        public override bool FindRecord(MessageTuple tuple, out MessageRecord record)
        {
            ...
            for (uint i = 0; i < _messageCount; i++)
            {
                ...
                && (recordPtr[2] == tuple.cond) && (recordPtr[3] == tuple.seq))
                {
                    ...
                    record.@string = ScummHelper.GetText(_data, recordPtr.ReadUInt16(5));
                    return true;
                }
                recordPtr.Offset += _recordSize;
            }
    ...
    internal class MessageState
    {
        ...
        private MessageRecord GetRecord(CursorStack stack, bool recurse)
        {
            ...
            if (res == null)
            {
                // TODO: warning("Failed to open message resource %d", stack.getModule());
                return null;
            }
            ...
            if (!reader.Init())
            {
                // TODO: warning("Message: failed to read resource header");
                return null;
            }
