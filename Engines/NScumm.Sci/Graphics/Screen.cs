...
namespace NScumm.Sci.Graphics
{
    ...
    internal enum GfxScreenMasks
    {
        ...
        CONTROL = 4,
        DISPLAY = 8,
        // not official sierra sci, only used internally
    ...
    internal class GfxScreen
    {
        ...
        public int _picNotValid;
        // possible values 0, 1 and 2
        public int _picNotValidSci11;
        // another variable that is used by kPicNotValid in sci1.1
        ...
        private static readonly UpScaledAdjust[] s_upscaledAdjustTable = {
            new UpScaledAdjust {
                gameHiresMode = GfxScreenUpscaledMode.S640x480,
                viewNativeRes = Sci32ViewNativeResolution.R640x400,
                numerator = 5,
                denominator = 6
            }
        };
        ...
        public void Scale2x(byte[] src, byte[] dest, int srcWidth, int srcHeight, byte bytesPerPixel = 1)
        {
            //assert(bytesPerPixel == 1 || bytesPerPixel == 2);
            int newWidth = srcWidth * 2;
            int pitch = newWidth * bytesPerPixel;
            var srcPtr = new ByteAccess(src);
            var dst = new ByteAccess(dest);
        ...
        public GfxScreen(ResourceManager resMan)
        {
            ...
            if ((SciEngine.Instance.Language == Core.Language.JA_JPN) && (ResourceManager.GetSciVersion() <= SciVersion.V1_1))
        ...
        public void AdjustBackUpscaledCoordinates(ref short y, ref short x, Sci32ViewNativeResolution viewNativeRes = Sci32ViewNativeResolution.NONE)
        {
            for (int i = 0; i < s_upscaledAdjustTable.Length; i++)
            {
                ...
                {
                    y = (short)((y * s_upscaledAdjustTable[i].denominator) / s_upscaledAdjustTable[i].numerator);
                    break;
                }
            }
        ...
        public void DebugShowMap(int mapNo)
        {
        ...
        public void DitherForceDitheredColor(int color)
        {
        ...
        {
            SciEngine.Instance.System.GraphicsManager.CopyRectToScreen(_activeScreen, _displayWidth, 0, 0, _displayWidth, _displayHeight);
        }
