    ...
    internal class GfxText16
    {
        ...
        public void Box(string text, ushort languageSplitter, bool show, Rect rect, short alignment, int fontId)
        {
            ...
            while (curTextPos < text.Length)
            {
                ...
                switch (alignment)
                {
                    ...
                    default:
                        Warning($"Invalid alignment {alignment} used in TextBox()");
                        break;
                }
        ...
        public Register AllocAndFillReferenceRectArray()
        {
            ...
            if (rectCount != 0)
            {
                ...
                {
                    var left = (short)_codeRefRects[curRect].Left;
                    var top = (short)_codeRefRects[curRect].Top;
                    var right = (short)_codeRefRects[curRect].Right;
                    var bottom = (short)_codeRefRects[curRect].Bottom;
