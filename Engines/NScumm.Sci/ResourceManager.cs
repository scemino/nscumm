    ...
    internal partial class ResourceManager
    {
        ...
        public ResourceSource.Resource FindResource(ResourceId id, bool locked)
        {
            ...
            if (retval.data != null)
                ...
                // TODO: warning("resMan: Failed to read %s", retval._id.toString().c_str());
                return null;
            }
        }
...
# if ENABLE_SCI32
        ...
        private ResourceErrorCodes ReadAudioMapSCI11(IntMapResourceSource map)
        {
            ...
            //if (!mapRes)
            //{
            //    // TODO: warning("Failed to open %i.MAP", map._volumeNumber);
            //    return ResourceErrorCodes.RESMAP_NOT_FOUND;
            //}
        ...
        private void ReadResourceMapSCI1(ExtMapResourceSource extMapResourceSource)
        {
            throw new NotImplementedException();
        }
        ...
        private ResourceErrorCodes ReadResourceMapSCI0(ExtMapResourceSource map)
        {
            ...
            if (map._resourceFile != null)
            {
                ...
                var path = ScummHelper.LocatePath(_directory, map.LocationName);
                if (path == null)
                    return ResourceErrorCodes.RESMAP_NOT_FOUND;
            ...
            do
            {
                ...
                {
                    fileStream.Dispose();
                    // TODO: warning("Error while reading %s", map.getLocationName().c_str());
                    return ResourceErrorCodes.RESMAP_NOT_FOUND;
                }
                ...
                ResourceId resId = new ResourceId(type, number);
                ...
                if (_resMap.ContainsKey(resId) == false)
                {
                    ...
                    if (source == null)
                    {
                        // TODO: warning("Could not get volume for resource %d, VolumeID %d", id, offset >> bShift);
                        if (_mapVersion != _volVersion)
                        {
                            // TODO: warning("Retrying with the detected volume version instead");
                            // TODO: warning("Map version was: %d, retrying with: %d", _mapVersion, _volVersion);
                            _mapVersion = _volVersion;
        ...
        private ViewType DetectViewType()
        {
            ...
            // TODO: warning("resMan: Couldn't find any views");
            return ViewType.Unknown;
        }
        ...
        private void AddToLRU(ResourceSource.Resource res)
        {
            ...
            {
                // TODO: warning("resMan: trying to enqueue resource with state %d", res._status);
                return;
            }
            _LRU.Insert(0, res);
	...
	debug("Adding %s.%03d (%d bytes) to lru control: %d bytes total",
        ...
        private void RemoveFromLRU(ResourceSource.Resource res)
        {
            ...
            {
                // TODO: warning("resMan: trying to remove resource that isn't enqueued");
                return;
            }
            _LRU.Remove(res);
            _memoryLRU -= (int)res.size;
