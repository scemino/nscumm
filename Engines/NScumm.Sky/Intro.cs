...
namespace NScumm.Sky
{
	partial class Intro : IDisposable
	{
        ...
        public Intro (Disk disk, Screen screen, MusicBase music, Sound sound, Text skyText, IMixer mixer, ISystem system)
		{
			_skyDisk = disk;
			_skyScreen = screen;
			_skyMusic = music;
			...
			case PLAYVOICE:
				{
					...
					vData.Set (0, 127, DataFileHeader.Size);
			...
			while ((data [i] != COMMANDEND) || _skyScreen.SequenceRunning ()) {
				while ((_skyScreen.SeqFramesLeft () < data [i])) {
					...
					case IC_PREPARE_TEXT:
						...
						break;
					case IC_SHOW_TEXT:
						var header = new DataFileHeader (_textBuf);
						header.s_x = data [i++];
						header.s_y = data [i++];
						ShowTextBuf ();
						break;
		...
		{
			var header = new DataFileHeader (_saveBuf);
			ushort x = header.s_x;
			ushort y = header.s_y;
			ushort width = header.s_width;
			ushort height = header.s_height;
