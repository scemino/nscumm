...
namespace NScumm.Sky
{
	internal partial class Text
	{
		...
		private const int NoOfTextSections = 8;
		...
		private uint _dtCharSpacing;
		//character separation adjustment
		...
		public DisplayedText DisplayText (string text, byte[] dest, bool center, ushort pixelWidth, byte color)
		{
			...
			//make the header
			var header = new DataFileHeader (dest);
			header.s_width = pixelWidth;
			header.s_height = (ushort)(_charHeight * numLines);
			header.s_sp_size = (ushort)(pixelWidth * _charHeight * numLines);
			header.s_offset_x = 0;
			header.s_offset_y = 0;
			...
			var ret = new DisplayedText {
				TextData = new DataFileHeader (dest),
		...
		{
			var header = new DataFileHeader (sprData);
			var offset = DataFileHeader.Size;
		...
		public DisplayedText LowTextManager (uint textNum, ushort width, ushort logicNum, byte color, bool center)
		{
			...
			SkyEngine.ItemList [cpt.Core.flag] = textInfo.TextData.Data;
	...
	{
		public DataFileHeader TextData;
