...
{
    public class AdLibPlayer: IPlayer, IMixerAudioStream, IDisposable
    {
        public AdLibPlayer(ScummEngine scumm, IMixer mixer)
        {
            _mutex = new object();
            _vm = scumm;
            _mixer = mixer;
            ...
            _registerBackUpTable = new byte[256];
            WriteReg(0x01, 0x00);
            WriteReg(0xBD, 0x00);
            WriteReg(0x08, 0x00);
            WriteReg(0x01, 0x20);
            ...
            _hwChannels = new HardwareChannel[9];
            _numHWChannels = _hwChannels.Length;
            _voiceChannels = new VoiceChannel[9];
        ...
        public void StartSound(int sound)
        {
            lock (_mutex)
            {
                ...
                else
                {
                    ...
                    if (sfx == null)
                    {
                        Debug.WriteLine("AdLib: No free sfx slot for sound {0}", sound);
                        return;
                    }
...
//                                    _vm->_res->lock(rtSound, sound);
        ...
        public int MusicTimer { get { return _engineMusicTimer; } }
        ...
        int ReadBuffer(short buffer, int numSamples)
        {
            lock (_mutex)
            {
                ...
                while (len > 0)
                {
                    ...
                    var samplesToRead = Math.Min(len, _samplesTillCallback);
        ...
        int AllocateHWChannel(int priority, SfxSlot owner = null)
        {
            ...
            if (channel != -1)
            {
                ...
                if (_hwChannels[channel].Allocated && _hwChannels[channel].SfxOwner != 0)
                {
        ...
        void SetupChannel(int channel, byte[] instr, int offset)
        {
            offset += 2;
            WriteReg(0xC0 + channel, instr[offset++]);
            SetupOperator(_operatorOffsetTable[channel * 2 + 0], instr, ref offset);
            SetupOperator(_operatorOffsetTable[channel * 2 + 1], instr, ref offset);
        }

        void SetupOperator(int opr, byte[] instr, ref int offset)
        {
            WriteReg(0x20 + opr, instr[offset++]);
            WriteReg(0x40 + opr, instr[offset++]);
            WriteReg(0x60 + opr, instr[offset++]);
            WriteReg(0x80 + opr, instr[offset++]);
            WriteReg(0xE0 + opr, instr[offset++]);
        }

        static readonly int[] _operatorOffsetTable = new int[18]
        {
            0,  3,  1,  4,
            2,  5,  8, 11,
            9, 12, 10, 13,
...
//            _vm->_res->unlock(rtSound, _soundPlaying);
        ...
        bool ParseCommand()
        {
            ...
            if (command == 0xFF)
            {
                ...
                else if (command == 81)
                {
                    ...
                    var timing = _musicData[_curOffset + 2] | (_musicData[_curOffset + 1] << 8);
            ...
            else
            {
                if (command >= 0x90)
                {
                    ...
                    var instrOffset = _instrumentOffset[command];
                    if (instrOffset != 0)
                    {
                        ...
                        else
                        {
                            ...
                            var channel = AllocateHWChannel(256);
                            if (channel != -1)
                            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            var rhythmInstr = _musicData[instrOffset + 13];
                            if (rhythmInstr < 6)
                            {
        ...
        void SetupFrequency(int channel, sbyte frequency)
        {
            ...
            var noteFrequency = _noteFrequencies[frequency];
            octave <<= 2;
        ...
        bool StartSfx(SfxSlot sfx, byte[] resource)
        {
            ...
            sfx.Channels[0].ResourceData = resource;
            ...
            while ((command = resource[bufferPosition]) != 0xFF)
            {
                switch (command)
                {
                    ...
                    default:
                        ...
                        if (curChannel >= 3)
                        {
                            Console.Error.WriteLine("AD SFX resource {0} uses more than 3 channels", sfx.Resource);
                        }
...
//            _vm->_res->unlock(rtSound, sfx->resource);
        ...
        void ParseSlot(Channel channel)
        {
            while (true)
            {
                var res = channel.ResourceData;
                int curOffset = channel.CurrentOffset;

                switch (res[curOffset])
                {
                    case 1:
                            // INSTRUMENT DEFINITION
                        ++curOffset;
                        channel.InstrumentData[0] = res[curOffset];
                        channel.InstrumentData[1] = res[curOffset + 2];
                        channel.InstrumentData[2] = res[curOffset + 9];
                        channel.InstrumentData[3] = res[curOffset + 8];
                        channel.InstrumentData[4] = res[curOffset + 4];
                        channel.InstrumentData[5] = res[curOffset + 3];
                        channel.InstrumentData[6] = 0;

                        SetupChannel(channel.HardwareChannel, channel.ResourceData, curOffset);

                        WriteReg(0xA0 + channel.HardwareChannel, res[curOffset + 0]);
                        WriteReg(0xB0 + channel.HardwareChannel, res[curOffset + 1] & 0xDF);
                    ...
                    case 2:
                        ...
                        ParseNote(channel.Notes[0], channel, channel.ResourceData, curOffset + 0);
                        ParseNote(channel.Notes[1], channel, channel.ResourceData, curOffset + 5);
                        return;
        ...
        void UpdateSlot(Channel channel)
        {
            ...
            for (int num = 0; num <= 1; ++num, curOffset += 5)
            {
                ...
                else
                {
                    ...
                    if (note.Bias != 0)
                    {
                        WriteRegisterSpecial(channel.HardwareChannel, note.Bias - note.InstrumentValue, res[curOffset] & 0x07);
                    }
                    else
                    {
                        WriteRegisterSpecial(channel.HardwareChannel, note.InstrumentValue, res[curOffset] & 0x07);
                    }
                }

                if (updateNote)
                {
                    if (ProcessNote(note, channel, res, curOffset))
                    {
                        if (0 == (res[curOffset] & 0x08))
                        {
                            channel.CurrentOffset += 11;
                            channel.State = ChannelState.Parse;
                            continue;
                        }
                        else if ((res[curOffset] & 0x10) != 0)
                        {
                            NoteOffOn(channel.HardwareChannel);
                        }

                        note.State = NoteState.PreInit;
                        ProcessNote(note, channel, res, curOffset);
                    }
                }

                if ((res[curOffset] & 0x20) != 0 && --note.PlayTime == 0)
                {
                    channel.CurrentOffset += 11;
                    channel.State = ChannelState.Parse;
                }
        ...
        void ParseNote(Note note, Channel channel, byte[] data, int offset)
        {
            if ((data[offset] & 0x80) != 0)
            {
                note.State = NoteState.PreInit;
                ProcessNote(note, channel, data, offset);
                note.PlayTime = 0;

                if ((data[offset] & 0x20) != 0)
                {
                    note.PlayTime = (data[4] >> 4) * 118;
                    note.PlayTime += (data[4] & 0x0F) * 8;
                }
            }
        }

        bool ProcessNote(Note note, Channel channel, byte[] dataReceivedEventArgs, int offset)
        {
            ...
            int instrumentDataOffset = dataReceivedEventArgs[offset] & 0x07;
            ...
            if (note.State == NoteState.Sustain)
            {
                note.SustainTimer = _numStepsTable[dataReceivedEventArgs[offset + 3] >> 4];

                if ((dataReceivedEventArgs[offset] & 0x40) != 0)
                {
            ...
            else
            {
                ...
                {
                    timer1 = dataReceivedEventArgs[offset + 3] & 0x0F;
                    timer2 = 0;
                }
                else
                {
                    timer1 = dataReceivedEventArgs[offset + (int)note.State + 1] >> 4;
                    timer2 = dataReceivedEventArgs[offset + (int)note.State + 1] & 0x0F;
                }
        ...
        {
            byte regValue = ReadReg(0xB0 | channel);
            WriteReg(0xB0 | channel, regValue & 0xDF);
            WriteReg(0xB0 | channel, regValue | 0x20);
        }

        void WriteRegisterSpecial(int channel, int value, int offset)
        {
            if (offset == 6)
            {
                return;
            ...
            var regValue = ReadReg(regNum) & (~_registerMaskTable[offset]);
        ...
        byte ReadRegisterSpecial(int channel, byte defaultValue, int offset)
        {
            ...
            if (_useOperatorTable[offset])
            {
                regNum = (byte)_operatorOffsetTable[_channelOperatorOffsetTable[offset] + channel * 2];
            }
            else
            {
                regNum = (byte)_channelOffsetTable[channel];
            }
        ...
        HardwareChannel[] _hwChannels;
        VoiceChannel[] _voiceChannels;

        // AdLib register utilities
        byte[] _registerBackUpTable;
        ...
        readonly int[] _instrumentOffset = new int[16];
        byte[] _musicData;
        ...
        int _musicTicks;
        bool _loopFlag;
        bool _musicLoopStart;

        static readonly int[] _operatorOffsetToChannel = new int[]
        {
            0,  1,  2,  0,  1,  2, -1, -1,
            3,  4,  5,  3,  4,  5, -1, -1,
            6,  7,  8,  6,  7,  8
        ...
        static readonly int[] _noteFrequencies = new int[]
        {
        ...
        static readonly int[] _rhythmOperatorTable = new int[]
        {
            0x00, 0x00, 0x14, 0x12, 0x15, 0x11
        };

        static readonly int[] _rhythmChannelTable = new int[]
        {
            0x00, 0x00, 0x07, 0x08, 0x08, 0x07
        };

        static readonly int[] _mdvdrTable = new int[]
        {
            0x00, 0x10, 0x08, 0x04, 0x02, 0x01
        };

        static readonly int[] _noteBiasTable = new int[]
        {
            0x00, 0x00, 0x3F, 0x00, 0x3F, 0x00, 0x00
        };

        static readonly int[] _numStepsTable = new int[]
        {
            1,    4,    6,    8,
            10,   14,   18,   24,
            36,   64,  100,  160,
        ...
        static readonly int[] _noteAdjustTable = new int[]
        {
            0,  4369,  8738, 13107,
        ...
        static readonly int[] _noteAdjustScaleTable = new int[]
        {
            255,   7,  63,  15,  63,  15,  63
        };

        static readonly int[] _baseRegisterTable = new int[]
        {
            0xA0, 0xC0, 0x40, 0x20, 0x40, 0x20, 0x00
        };

        static readonly bool[] _useOperatorTable = new bool[]
        {
            false, false, true, true, true, true, false
        };

        static readonly int[] _channelOffsetTable = new int[]
        {
            0,  1,  2,  3,
            4,  5,  6,  7,
            8,  8,  7
        ...
        static readonly int[] _channelOperatorOffsetTable = new int[]
        {
            0, 0, 1, 1, 0, 0, 0
        };

        static readonly int[] _registerMaskTable = new int[]
        {
            0xFF, 0x0E, 0x3F, 0x0F, 0x3F, 0x0F, 0x00
        };

        static readonly int[] _registerShiftTable = new int[]
        {
            0, 1, 0, 0, 0, 0, 0
        };
        ...
        class Note
        {
            ...
            public int SustainTimer{ get; set; }

            public int InstrumentValue{ get; set; }

            public int Bias{ get; set; }

            public int PreIncrease{ get; set; }
        ...
        class Channel
        {
            ...
            public byte[] InstrumentData { get; private set; }
        ...
        class HardwareChannel
        {
            ...
            public SfxSlot SfxOwner { get; set; }
        }

        class VoiceChannel
        {
            public uint LastEvent { get; set; }

            public uint Frequency { get; set; }

            public int B0Reg { get; set; }
        }
    }
}
