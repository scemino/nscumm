...
namespace NScumm.Core.Audio
{
    ...
    public class Module
    {
        ...
        public readonly sample_t[] sample = new sample_t[NUM_SAMPLES];
        public readonly sample_offs[] commonSamples = new sample_offs[NUM_SAMPLES];
        ...
        {
            ScummHelper.MakeTag('M', '.', 'K', '.'), ScummHelper.MakeTag('M', '!', 'K', '!'),
            ScummHelper.MakeTag('F', 'L', 'T', '4')
        };
        ...
        public bool Load(Stream st, int offs)
        {
            ...
            for (int i = 0; i < NUM_SAMPLES; ++i)
            {
                ...
                sample[i].data = sample[i].len == 0 ? null : br.ReadBytes(sample[i].len);
            }

            return true;
        }

        private static byte PeriodToNote(short period, byte finetune = 0)
        {
            var diff1 = (short) Math.Abs(periods[finetune, 0] - period);
            if (diff1 == 0)
                return 0;

            for (int i = 1; i < 60; i++)
            {
                var diff2 = (short) Math.Abs(periods[finetune, i] - period);
                if (diff2 == 0)
                    return (byte) i;
                if (diff2 > diff1)
                    return (byte) (i - 1);
