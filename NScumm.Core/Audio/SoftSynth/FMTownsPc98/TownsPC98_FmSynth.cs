...
namespace NScumm.Core.Audio.SoftSynth
{
    ...
    abstract class TownsPC98_FmSynth: IAudioStream
    {
        ...
        public int ReadBuffer(short[] buffer, int count)
        {
            ...
            var bufferOffset = 0;
            var tmpOffset = 0;
            while (_ready && samplesLeft != 0)
            {
                int render = samplesLeft;

                for (int i = 0; i < 2; i++)
                {
                    if (_timers[i].enabled && _timers[i].cb != null)
                    {
                        if (_timers[i].smpTillCb == 0)
                        {
                        ...
                        render = Math.Min(render, _timers[i].smpTillCb);
                    }
                }
                ...
                NextTick(tmp, tmpOffset, (uint)render);

                if (_ssg != null)
                    _ssg.NextTick(tmp, tmpOffset, (uint)render);
#if !DISABLE_PC98_RHYTHM_CHANNEL
                if (_prc != null)
                    _prc.NextTick(tmp, tmpOffset, render);
#endif

                NextTickEx(tmp, tmpOffset, render);

                for (int i = 0; i < render; ++i)
                {
                    int l = ScummHelper.Clip(tmp[tmpOffset + (i << 1)], -32767, 32767);
                    buffer[bufferOffset + (i << 1)] = (short)l;
                    int r = ScummHelper.Clip(tmp[tmpOffset + (i << 1) + 1], -32767, 32767);
                    buffer[bufferOffset + (i << 1) + 1] = (short)r;
                }

                bufferOffset += (render << 1);
                tmpOffset += (render << 1);
            }
        ...
        public void WriteReg(byte part, byte regAddress, int value)
        {
            ...
            lock (_mutex)
            {
                ...
                byte[] oprOrdr = { 0, 2, 1, 3 };

                byte h = (byte)(regAddress & 0xf0);
                byte l = (byte)(regAddress & 0x0f);

                ChanInternal c = null;
                TownsPC98_FmSynthOperator[] co = null;
                TownsPC98_FmSynthOperator o = null;
                ...
                {
                    c = _chanInternal[(value & 3) + (((value & 4) != 0) ? 3 : 0)];
                    co = c.opr;
                }
                ...
                switch (h)
                {
                    ...
                    case 0x20:
                        ...
                        {
                            // Timer A
                            _timers[0].value = (ushort)((_timers[0].value & 3) | (value << 2));
                        }
                        else if (l == 5)
                        {
                            // Timer A
                            _timers[0].value = (ushort)((_timers[0].value & 0x3fc) | (value & 3));
                        }
                        else if (l == 6)
                        {
                            // Timer B
                            _timers[1].value = (ushort)(value & 0xff);
                        }
                        else if (l == 7)
                        {
                            if ((value & 1) != 0)
                        ...
                        else if (l == 2)
                        {
                            // LFO
                            if ((value & 8) != 0)
                                Debug.WriteLine("TownsPC98_FmSynth: TRYING TO USE LFO (NOT SUPPORTED)");
                        }
                        else if (l == 10 || l == 11)
                        {
                            // DAC
                            if (l == 11 && ((value & 0x80) != 0))
                                Debug.WriteLine("TownsPC98_FmSynth: TRYING TO USE DAC (NOT SUPPORTED)");
                        }
                        break;
                    ...
                    case 0x30:
                        // detune, multiple
                        o.Detune((value >> 4) & 7);
                        o.Multiple((uint)(value & 0x0f));
                        c.updateEnvelopeParameters = true;
                        break;

                    case 0x40:
                        // total level
                        o.TotalLevel((uint)(value & 0x7f));
                        break;

                    case 0x50:
                        // rate scaling, attack rate
                        o.AttackRate((uint)(value & 0x1f));
                        if (o.ScaleRate((byte)(value >> 6)))
                            c.updateEnvelopeParameters = true;
                        break;

                    case 0x60:
                        // first decay rate, amplitude modulation
                        o.DecayRate((uint)(value & 0x1f));
                        o.AmpModulation((value & 0x80) != 0);
                        break;

                    case 0x70:
                        // secondary decay rate
                        o.SustainRate((uint)(value & 0x1f));
                        break;

                    case 0x80:
                        // secondary amplitude, release rate;
                        o.SustainLevel((uint)(value >> 4));
                        o.ReleaseRate((uint)(value & 0x0f));
                        break;

                    case 0x90:
                        Debug.WriteLine("TownsPC98_FmSynth: TRYING TO USE SSG ENVELOPE SHAPES (NOT SUPPORTED)");
                        break;
                    ...
                    case 0xa0:
                        ...
                        {
                            c.frqTemp = (ushort)((c.frqTemp & 0xff) | (value << 8));
                        }
                        else if (l == 8)
                        {
                            // Ch 3/6 special mode frq
                            Debug.WriteLine("TownsPC98_FmSynth: TRYING TO USE CH 3/6 SPECIAL MODE FREQ (NOT SUPPORTED)");
                        }
                        else if (l == 12)
                        {
                            // Ch 3/6 special mode frq
                    ...
                    case 0xb0:
                        ...
                        if (l == 0)
                        {
                            ...
                            c.algorithm = (byte)(value & 7);
                        }
                        else if (l == 4)
                        {
                            // stereo, LFO sensitivity
                            c.enableLeft = ((value & 0x80) != 0);
                            c.enableRight = ((value & 0x40) != 0);
                            c.ampModSensitivity((value & 0x30) >> 4);
                            c.frqModSensitivity((uint)(value & 3));
                        }
                        break;

                    default:
                        Debug.WriteLine("TownsPC98_FmSynth: UNKNOWN ADDRESS {0}", regAddress);
                        break;
                }
            }
        }
        ...
        public abstract void TimerCallbackB();
        ...
        public virtual bool Init()
        {
            ...
            {
                _chanInternal[i] = new ChanInternal();
                for (int j = 0; j < 4; ++j)
                    _chanInternal[i].opr[j] = new TownsPC98_FmSynthOperator(_timerbase, (uint)_rtt, _oprRates, _oprRateshift, _oprAttackDecay, _oprFrq, _oprSinTbl, _oprLevelOut, _oprDetune);
            }

            if (_numSSG != 0)
            {
                _ssg = new TownsPC98_FmSynthSquareSineSource(_timerbase, (uint)_rtt);
            ...
            if (_hasPercussion)
            {
                _prc = new TownsPC98_FmSynthPercussionSource(_timerbase, (uint)_rtt);
                _prc.Init(_percussionData);
            }
            #endif

            _timers[0].cb = TimerCallbackA;
            _timers[1].cb = TimerCallbackB;
        ...
        protected virtual void NextTickEx(int[] buffer, int offset, int bufferSize)
        {
        }
        ...
        protected void Reset()
        {
            lock (_mutex)
            {
                for (int i = 0; i < _numChan; i++)
                {
                    ...
                    Array.Clear(_chanInternal[i].feedbuf, 0, 3);
        ...
        protected void SetVolumeIntern(int volA, int volB)
        {
            lock (_mutex)
            {
                _volumeA = (ushort)ScummHelper.Clip(volA, 0, Mixer.MaxMixerVolume);
                _volumeB = (ushort)ScummHelper.Clip(volB, 0, Mixer.MaxMixerVolume);
                if (_ssg != null)
        ...
        protected void ToggleRegProtection(bool prot)
        {
            _regProtectionFlag = prot;
        }

        void NextTick(int[] buffer, int offset, uint bufferSize)
        {
            if (!_ready)
                return;
            ...
            for (int i = 0; i < _numChan; i++)
            {
                ...
                for (uint ii = 0; ii < bufferSize; ii++)
                {
                    ...
                    int finOut = (output << 2) / ((_numChan + _numSSG - 3) / 3);
        ...
        void GenerateTables()
        {
            _oprRates = new byte[128];

            _oprRates.WriteUInt32BigEndian(32, _numChan == 6 ? 0x90900000 : 0x00081018);
            _oprRates.WriteUInt32BigEndian(36, _numChan == 6 ? 0x00001010U : 0x00081018);
            _oprRates.Set(0, 0x90, 32);
            _oprRates.Set(96, 0x80, 32);
            var dst = 40;
            for (int i = 0; i < 40; i += 4)
            {
                _oprRates.WriteUInt32BigEndian(dst + i, 0x00081018);
            }
            ...
            for (var i = 0; i < 16; i++)
            {
                byte v = (i < 12) ? (byte)i : (byte)12;
                _oprRates[dst++] = (byte)((4 + v) << 3);
            }
        ...
        protected TownsPC98_FmSynthSquareSineSource _ssg;
        ...
        class ChanInternal
        {
            ...
            public void frqModSensitivity(uint value)
            {
                frqModSvty = value << 5;
            }
            ...
            public int[] feedbuf = new int[3];
            public byte algorithm;

            public uint ampModSvty;
            public uint frqModSvty;

            public TownsPC98_FmSynthOperator[] opr = new TownsPC98_FmSynthOperator[4];
        }
        ...
        delegate void ChipTimerProc();


        protected readonly int _numChan;
        protected readonly int _numSSG;
        readonly bool _hasPercussion;
        ...
        class ChipTimer
        {
            ...
            public int smpTillCb;
            public uint smpTillCbRem;
            public int smpPerCb;
            public uint smpPerCbRem;

            public ChipTimerProc cb;
        }

        ChipTimer[] _timers = new ChipTimer[2];

        protected int _volMaskA, _volMaskB;
        ushort _volumeA, _volumeB;
        ...
        static readonly int[][] _ssgTables = new int[2][]
        {
            new int[]
            {
        ...
        #if !DISABLE_PC98_RHYTHM_CHANNEL
        static readonly byte[] _percussionData =
            {
