...
namespace NScumm.Core.Audio.SoftSynth.Mt32
{
    class PartialManager
    {
        Synth synth;
        Part[] parts;
        byte[] numReservedPartialsForPart = new byte[9];
        Partial[] partialTable;
        Poly[] freePolys;
        int firstFreePolyIndex;
        
        public PartialManager(Synth synth, Part[] parts)
        {
            this.synth = synth;
            this.parts = parts;

            partialTable = new Partial[synth.PartialCount];
            freePolys = new Poly[synth.PartialCount];
            for (int i = 0; i < synth.PartialCount; i++)
            {
        ...
        public Partial GetPartial(int partialNum)
        {
            if (partialNum > synth.PartialCount - 1)
            {
                return null;
            }
        ...
        public int SetReserve(BytePtr rset)
        {
            int pr = 0;
            for (int x = 0; x <= 8; x++)
            {
        ...
        {
            if (firstFreePolyIndex < synth.PartialCount)
            {
        ...
        public Partial AllocPartial(int partNum)
        {
            ...
            for (int partialNum = 0; partialNum < synth.PartialCount; partialNum++)
            {
        ...
        public bool FreePartials(int needed, int partNum)
        {
...
# if MT32EMU_QUIRK_FREE_PARTIALS_MT32
...
# if MT32EMU_QUIRK_FREE_PARTIALS_MT32
        ...
        private bool AbortFirstPolyPreferHeldWhereReserveExceeded(int minPart)
        {
            ...
            for (int partNum = 7; partNum >= minPart; partNum--)
            {
                ...
                if (parts[usePartNum].ActivePartialCount > numReservedPartialsForPart[usePartNum])
                {
        ...
        public void PolyFreed(Poly poly)
        {
            if (0 == firstFreePolyIndex)
            {
                ...
                for (var partNum = 0; partNum < 9; partNum++)
                {
                    Poly activePoly = synth.GetPart(partNum).GetFirstActivePoly();
                    int polyCount = 0;
        ...
        private bool AbortFirstReleasingPolyWhereReserveExceeded(int minPart)
        {
            ...
            for (int partNum = 7; partNum >= minPart; partNum--)
            {
                ...
                if (parts[usePartNum].ActivePartialCount > numReservedPartialsForPart[usePartNum])
                {
                    ...
                    if (parts[usePartNum].AbortFirstPoly( PolyState.Releasing))
                    {
                        return true;
                    }
                }
        ...
        public int GetFreePartialCount()
        {
            int count = 0;
            for (int i = 0; i < synth.PartialCount; i++)
            {
        ...
        public bool ProduceOutput(int i, Ptr<Sample> leftBuf, Ptr<Sample> rightBuf, int bufferLength)
        {
