...
namespace NScumm.Core.Audio.SoftSynth.Mt32
{
    public class Tables
    {
        static readonly byte[] resAmpDecayFactorTable = { 31, 16, 12, 8, 5, 3, 2, 1 };
        ...
        public byte[] levelToAmpSubtraction = new byte[101];

        // CONFIRMED: ...
        public byte[] envLogarithmicTime = new byte[256];

        // CONFIRMED: ...
        public byte[] masterVolToAmpSubtraction = new byte[101];

        // CONFIRMED:
        public byte[] pulseWidth100To255 = new byte[101];

        public ushort[] exp9 = new ushort[512];
        public ushort[] logsin9 = new ushort[512];

        public byte[] resAmpDecayFactor;

        private Tables()
        {
            for (var lf = 0; lf <= 100; lf++)
            {
                // CONFIRMED:KG: This matches a ROM table found by Mok
                float fVal = (float)((2.0f - Math.Log10(lf + 1.0f)) * 128.0f);
                int val = (int)(fVal + 1.0);
                if (val > 255)
                {
                    val = 255;
                }
                levelToAmpSubtraction[lf] = (byte)val;
            }

            envLogarithmicTime[0] = 64;
            for (var lf = 1; lf <= 255; lf++)
            {
                // CONFIRMED:KG: This matches a ROM table found by Mok
                envLogarithmicTime[lf] = (byte)Math.Ceiling(64.0f + Math.Log((float)lf) * 8.0f);
            }

# if EMULATE_LAPC_I // Dummy #ifdef - we'll have runtime emulation mode selection in future.
            ...
            {
                masterVolToAmpSubtraction[masterVol] = (byte)(106.31 - 16.0f * MT32EmuMath.LOG2F(masterVol));
            }
#endif

            for (int i = 0; i <= 100; i++)
            {
                pulseWidth100To255[i] = (byte)(i * 255 / 100.0f + 0.5f);
            ...
            for (int i = 0; i < 512; i++)
            {
                exp9[i] = (ushort)(8191.5f - MT32EmuMath.EXP2F(13.0f + ~i / 512.0f));
            }

            // There is a logarithmic sine table inside the LA32 chip. The table contains 13-bit integer values.
            for (int i = 1; i < 512; i++)
            {
                logsin9[i] = (ushort)(0.5f - Math.Log(Math.Sin((i + 0.5f) / 1024.0f * Math.PI)) * 1024.0f);
            }
        ...
        public static readonly Tables Instance = new Tables();
    }
}
