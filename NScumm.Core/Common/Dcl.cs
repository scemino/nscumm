...
namespace NScumm.Core.Common
{
    public class DecompressorDCL
    {
        ...
        const int MIDI_SETUP_BUNDLE_FILE_MAXIMUM_DICTIONARY_SIZE = 4096;
        ...
        public bool Unpack(Stream sourceStream, Stream targetStream, uint targetSize, bool targetFixedSize)
        {
            ...
            while ((!targetFixedSize) || (_bytesWritten < _targetSize))
            {
                if (GetBitsLSB(1) != 0)
                {
                    ...
                    if (tokenLength == 2)
                        tokenOffset = (ushort) ((value << 2) | GetBitsLSB(2));
                    else
                    ...
                    if (_targetFixedSize)
                    {
                        ...
                        {
                            Warning(
                                "DCL-INFLATE Error: Write out of bounds while copying %d bytes (declared unpacked size is %d bytes, current is %d + %d bytes)",
                    ...
                    {
                        Warning(
                            "DCL-INFLATE Error: Attempt to copy from before beginning of input stream (declared unpacked size is %d bytes, current is %d bytes)",
                            _targetSize, _bytesWritten);
                        return false;
                    }

                    ushort dictionaryBaseIndex = (ushort) ((dictionaryPos - tokenOffset) & dictionaryMask);
                    ushort dictionaryIndex = dictionaryBaseIndex;
                    ushort dictionaryNextIndex = dictionaryPos;

                    while (tokenLength != 0)
                    {
                        // Write byte from dictionary
                        PutByte(dictionary[dictionaryIndex]);
                        // TODO: Debug(9, "\33[32;31m%02x\33[37;37m ", dictionary[dictionaryIndex]);

                        dictionary[dictionaryNextIndex] = dictionary[dictionaryIndex];

                        dictionaryNextIndex = (ushort) ((dictionaryNextIndex + 1) & dictionaryMask);
                        dictionaryIndex = (ushort) ((dictionaryIndex + 1) & dictionaryMask);
                ...
                else
                {
                    ...
                    // TODO: Debug(9, "\33[32;31m%02x \33[37;37m", value);
                }
            }
            ...
            if (_targetFixedSize)
            {
                if (_bytesWritten != _targetSize)
                    Warning(
                        $"DCL-INFLATE Error: Inconsistent bytes written ({_bytesWritten}) and target buffer size ({_targetSize})");
        ...
        /// </summary>
        /// <param name="sourceStream">sourceStream source stream to read from.</param>
        /// <param name="targetStream">targetStream target memory stream to write to.</param>
        /// <param name="targetSize">Target size.</param>
        /// <param name="targetFixedSize">Target fixed size.</param>
        private void Init(Stream sourceStream, Stream targetStream, uint targetSize, bool targetFixedSize)
        {
            _sourceStream = sourceStream;
            _targetStream = targetStream;
            _sourceSize = (uint) sourceStream.Length;
        ...
        private void FetchBitsLSB()
        {
            while (_nBits <= 24)
            {
                _dwBits |= ((uint) _sourceStream.ReadByte()) << _nBits;
                _nBits += 8;
                _bytesRead++;
            }
        }
        ...
        /// </summary>
        /// <returns>n-bits number.</returns>
        /// <param name="n">number of bits to get.</param>
        ...
        private static int BN(int pos, int left, int right)
        {
            return ((left << 12) | (right));
        }

        private static int LN(int pos, int value)
        {
            return ((value) | HUFFMAN_LEAF);
        }

        private static readonly int[] length_tree =
        {
            BN(0, 1, 2),
            BN(1, 3, 4), BN(2, 5, 6),
        ...
        private static readonly int[] distance_tree =
        {
            BN(0, 1, 2),
            BN(1, 3, 4), BN(2, 5, 6),
            //
        ...
        private static readonly int[] ascii_tree =
        {
        ...
        private int Huffman_lookup(int[] tree)
        {
            int pos = 0;

            while ((tree[pos] & HUFFMAN_LEAF) == 0)
            {
                int bit = (int) GetBitsLSB(1);
                // TODO: Debug(8, "[%d]:%d->", pos, bit);
