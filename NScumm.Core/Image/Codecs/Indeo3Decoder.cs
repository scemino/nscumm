...
namespace NScumm.Core.Image.Codecs
{
    ...
    ///  - VMDDecoder
    /// </summary>
    public class Indeo3Decoder : Codec
    {
        private Surface _surface;

        private PixelFormat _pixelFormat;

        private static readonly int[] _corrector_type_0 = {
            195, 159, 133, 115, 101,  93,  87,  77,
            195, 159, 133, 115, 101,  93,  87,  77,
            128,  79,  79,  79,  79,  79,  79,  79
        };
        private static readonly int[] _corrector_type_2 = { 9, 7, 6, 8, 5, 4, 3, 2 };
        ...
        struct YUVBufs
        {
            ...
            public BytePtr Ubuf;
            public BytePtr Vbuf;
            public BytePtr the_buf;
            public uint the_buf_size;
            public ushort y_w, y_h;
            public ushort uv_w, uv_h;
        }

        private YUVBufs[] _iv_frame = new YUVBufs[2];
        private Ptr<YUVBufs> _cur_frame;
        private Ptr<YUVBufs> _ref_frame;

        private BytePtr _ModPred;
        private Ptr<ushort> _corrector_type;

        public override PixelFormat PixelFormat => _pixelFormat;

        public Indeo3Decoder(ushort width, ushort height)
        {
            _pixelFormat = Engine.Instance.OSystem.GraphicsManager.PixelFormat;

            _surface = new Surface(width, height, _pixelFormat);

            BuildModPred();
            AllocFrames();
        }

        private void BuildModPred()
        {
            _ModPred = new byte[8 * 128];

            for (int i = 0; i < 128; i++)
            {
                _ModPred[i + 0 * 128] = (byte)((i > 126) ? 254 : 2 * ((i + 1) - ((i + 1) % 2)));
                _ModPred[i + 1 * 128] = (byte)((i == 7) ? 20 : ((i == 119 || i == 120)
                                              ? 236 : 2 * ((i + 2) - ((i + 1) % 3))));
                _ModPred[i + 2 * 128] = (byte)((i > 125) ? 248 : 2 * ((i + 2) - ((i + 2) % 4)));
                _ModPred[i + 3 * 128] = (byte)(2 * ((i + 1) - ((i - 3) % 5)));
                _ModPred[i + 4 * 128] = (byte)((i == 8) ? 20 : 2 * ((i + 1) - ((i - 3) % 6)));
                _ModPred[i + 5 * 128] = (byte)(2 * ((i + 4) - ((i + 3) % 7)));
                _ModPred[i + 6 * 128] = (byte)((i > 123) ? 240 : 2 * ((i + 4) - ((i + 4) % 8)));
                _ModPred[i + 7 * 128] = (byte)(2 * ((i + 5) - ((i + 4) % 9)));
            }

            _corrector_type = new ushort[24 * 256];
            ...
            for (int i = 0; i < 24; i++)
            {
                for (int j = 0; j < 256; j++)
                {
                    _corrector_type[i * 256 + j] =
                         (ushort)((j < _corrector_type_0[i]) ? 1 :
                      ((j < 248 || (i == 16 && j == 248)) ? 0 :
        ...
        private void AllocFrames()
        {
            int luma_width = (_surface.Width + 3) & (~3);
            int luma_height = (_surface.Height + 3) & (~3);

            int chroma_width = ((luma_width >> 2) + 3) & (~3);
            int chroma_height = ((luma_height >> 2) + 3) & (~3);

            int luma_pixels = luma_width * luma_height;
            int chroma_pixels = chroma_width * chroma_height;

            uint bufsize = (uint)(luma_pixels * 2 + luma_width * 3 +
                (chroma_pixels + chroma_width) * 4);

            _iv_frame[0].y_w = _iv_frame[1].y_w = (ushort)luma_width;
            _iv_frame[0].y_h = _iv_frame[1].y_h = (ushort)luma_height;
            _iv_frame[0].uv_w = _iv_frame[1].uv_w = (ushort)chroma_width;
            _iv_frame[0].uv_h = _iv_frame[1].uv_h = (ushort)chroma_height;

            _iv_frame[0].the_buf_size = bufsize;
            _iv_frame[1].the_buf_size = 0;

            _iv_frame[0].the_buf = new byte[bufsize];
            _iv_frame[1].the_buf = BytePtr.Null;
        ...
        public override Surface DecodeFrame(Stream stream)
        {
            // Not Indeo 3? Fail
            if (!IsIndeo3(stream))
                return null;
            ...
            uint frameDataLen = br.ReadUInt32();

            // Less data than the frame should have? Fail
            if (stream.Length < (int)(frameDataLen - 16))
                return null;

            stream.Seek(16, SeekOrigin.Begin); // Behind header
            stream.Seek(2, SeekOrigin.Current);  // Unknown

            ushort flags1 = br.ReadUInt16();
            uint flags3 = br.ReadUInt32();
            byte flags2 = br.ReadByte();

            // Finding the reference frame
            if ((flags1 & 0x200) != 0)
            {
                _cur_frame = new Ptr<YUVBufs>(_iv_frame, 1);
                _ref_frame = _iv_frame;
            }
            else
            {
                _cur_frame = _iv_frame;
                _ref_frame = new Ptr<YUVBufs>(_iv_frame, 1);
            }
            ...
            ushort fHeight = br.ReadUInt16();
            ushort fWidth = br.ReadUInt16();

            int chromaHeight = ((fHeight >> 2) + 3) & 0x7FFC;
            int chromaWidth = ((fWidth >> 2) + 3) & 0x7FFC;

            uint offs;
            uint offsY = br.ReadUInt32() + 16;
            uint offsU = br.ReadUInt32() + 16;
            uint offsV = br.ReadUInt32() + 16;

            stream.Seek(4, SeekOrigin.Current);

            var hPos = stream.Position;
            ...
            var dataSize = stream.Length - hPos;

            var inData = new byte[dataSize];

            if (stream.Read(inData, 0, (int)dataSize) != dataSize)
            {
                return null;
            }
            ...
            // Luminance Y
            stream.Seek(offsY, SeekOrigin.Begin);
            buf_pos = new BytePtr(inData, (int)(offsY + 4 - hPos));
            offs = br.ReadUInt32();
            DecodeChunk(_cur_frame.Value.Ybuf, _ref_frame.Value.Ybuf, fWidth, fHeight,
                    new BytePtr(buf_pos, (int)(offs * 2)), flags2, hdr_pos, buf_pos, Math.Min((int)fWidth, 160));

            // Chrominance U
            stream.Seek(offsU, SeekOrigin.Begin);
            buf_pos = new BytePtr(inData, (int)(offsU + 4 - hPos));
            offs = br.ReadUInt32();
            DecodeChunk(_cur_frame.Value.Vbuf, _ref_frame.Value.Vbuf, chromaWidth, chromaHeight,
                    new BytePtr(buf_pos, (int)(offs * 2)), flags2, hdr_pos, buf_pos, Math.Min(chromaWidth, 40));

            // Chrominance V
            stream.Seek(offsV, SeekOrigin.Begin);
            buf_pos = new BytePtr(inData, (int)(offsV + 4 - hPos));
            offs = br.ReadUInt32();
            DecodeChunk(_cur_frame.Value.Ubuf, _ref_frame.Value.Ubuf, chromaWidth, chromaHeight,
                    new BytePtr(buf_pos, (int)(offs * 2)), flags2, hdr_pos, buf_pos, Math.Min(chromaWidth, 40));

            BytePtr srcY = _cur_frame.Value.Ybuf;
            BytePtr srcU = _cur_frame.Value.Ubuf;
            BytePtr srcV = _cur_frame.Value.Vbuf;

            // Create buffers for U/V with an extra row/column copied from the second-to-last
            // row/column.
            var tempU = new byte[(chromaWidth + 1) * (chromaHeight + 1)];
            var tempV = new byte[(chromaWidth + 1) * (chromaHeight + 1)];

            for (var i = 0; i < chromaHeight; i++)
            {
                Array.Copy(srcU.Data, srcU.Offset + chromaWidth * i, tempU, (chromaWidth + 1) * i, chromaWidth);
                Array.Copy(srcV.Data, srcV.Offset + chromaWidth * i, tempV, (chromaWidth + 1) * i, chromaWidth);
                tempU[(chromaWidth + 1) * i + chromaWidth] = srcU[chromaWidth * (i + 1) - 1];
                tempV[(chromaWidth + 1) * i + chromaWidth] = srcV[chromaWidth * (i + 1) - 1];
            }

            Array.Copy(tempU, (chromaWidth + 1) * (chromaHeight - 1), tempU, (chromaWidth + 1) * chromaHeight, chromaWidth + 1);
            Array.Copy(tempV, (chromaWidth + 1) * (chromaHeight - 1), tempV, (chromaWidth + 1) * chromaHeight, chromaWidth + 1);

            // Blit the frame onto the surface
            int scaleWidth = _surface.Width / fWidth;
            int scaleHeight = _surface.Height / fHeight;

            if (scaleWidth == 1 && scaleHeight == 1)
            {
                // Shortcut: Don't need to scale so we can decode straight to the surface
                YUVToRGBManager.Convert410(_surface, LuminanceScale.ScaleITU, srcY, tempU, tempV,
                        fWidth, fHeight, fWidth, chromaWidth + 1);
            }
            else
            {
                // Need to upscale, so decode to a temp surface first
                Surface tempSurface = new Surface(fWidth, fHeight, _surface.PixelFormat);

                YUVToRGBManager.Convert410(tempSurface, LuminanceScale.ScaleITU, srcY, tempU, tempV,
                        fWidth, fHeight, fWidth, chromaWidth + 1);
                ...
                for (int y = 0; y < _surface.Height; y++)
                {
                    ...
                    {
                        if (_surface.PixelFormat.GetBytesPerPixel() == 1)
                            _surface.GetBasePtr(x, y).WriteByte(0, tempSurface.GetBasePtr(x / scaleWidth, y / scaleHeight).Value);
                        else if (_surface.PixelFormat.GetBytesPerPixel() == 2)
                            _surface.GetBasePtr(x, y).WriteUInt16(0, tempSurface.GetBasePtr(x / scaleWidth, y / scaleHeight).ToUInt16());
                        else if (_surface.PixelFormat.GetBytesPerPixel() == 4)
                            _surface.GetBasePtr(x, y).WriteUInt32(0, tempSurface.GetBasePtr(x / scaleWidth, y / scaleHeight).ToUInt32());
                    }
                }
            }
            return _surface;
        ...
        struct ustr_t
        {
            ...
            public int split_direction;
            public int usl7;
        }

        private static void RLE_V3_CHECK(ref BytePtr buf1, int rle_v1, int rle_v2, int rle_v3)
        {
            if (rle_v3 == 0)
            {
                rle_v2 = buf1.Value;
        ...
        private static void RLE_V2_CHECK(ref BytePtr buf1, int rle_v2, int rle_v3, int lp2)
        {
            rle_v2--;
            if (rle_v2 == 0)
            {
        ...
        private static void LP2_CHECK(ref BytePtr buf1, int rle_v3, int lp2)
        {
            if (lp2 == 0 && rle_v3 != 0)
                rle_v3 = 0;
            else
        ...
        private static void LV1_CHECK(ref BytePtr buf1, int rle_v3, int lv1, int lp2)
        {
            if ((lv1 & 0x80) != 0)
            {
                if (rle_v3 != 0)
        ...
        private void DecodeChunk(BytePtr cur, BytePtr @ref, int width, int height,
                BytePtr buf1, byte fflags2, BytePtr hdr,
                BytePtr buf2, int min_width_160)
        {
            byte bit_buf;
            int bit_pos, lv, lv1, lv2;
            Ptr<int> width_tbl;
            int[] width_tbl_arr = new int[10];
            BytePtr ref_vectors;
            BytePtr cur_frm_pos, ref_frm_pos, cp, cp2;
            UIntAccess cur_lp, ref_lp;
            Ptr<uint>[] correction_lp = new Ptr<uint>[2];
            Ptr<uint>[] correctionloworder_lp = new Ptr<uint>[2];
            Ptr<uint>[] correctionhighorder_lp = new Ptr<uint>[2];
            Ptr<ushort>[] correction_type_sp = new Ptr<ushort>[2];
            ustr_t[] strip_tbl = new ustr_t[20];
            int strip;
            ...
            bit_buf = 0;
            ref_vectors = BytePtr.Null;

            width_tbl = new Ptr<int>(width_tbl_arr, 4);
            ...
            strip_tbl[strip].ypos = strip_tbl[strip].xpos = 0;
            for (strip_tbl[strip].width = min_width_160; width > strip_tbl[strip].width; strip_tbl[strip].width *= 2)
                ;
            strip_tbl[strip].height = height;
            strip_tbl[strip].split_direction = 0;
            strip_tbl[strip].split_flag = 0;
            strip_tbl[strip].usl7 = 0;
            ...
            while (strip >= 0)
            {
                ...
                {
                    bit_pos = 8;
                    bit_buf = buf1.Value; buf1.Offset++;
                }
                ...
                {
                    strip++;
                    strip_tbl[strip] = strip_tbl[strip - 1];
                    strip_tbl[strip].split_flag = 1;
                    strip_tbl[strip].split_direction = 0;
                    strip_tbl[strip].height = (strip_tbl[strip].height > 8 ? ((strip_tbl[strip].height + 8) >> 4) << 3 : 4);
                    continue;
                }
                else if (cmd == 1)
                {
                    ...
                    strip_tbl[strip] = strip_tbl[strip - 1];
                    strip_tbl[strip].split_flag = 1;
                    strip_tbl[strip].split_direction = 1;
                    strip_tbl[strip].width = (strip_tbl[strip].width > 8 ? ((strip_tbl[strip].width + 8) >> 4) << 3 : 4);
                    continue;
                }
                else if (cmd == 2)
                {
                    if (strip_tbl[strip].usl7 == 0)
                    {
                        strip_tbl[strip].usl7 = 1;
                        ref_vectors = BytePtr.Null;
                        continue;
                    }
                }
                else if (cmd == 3)
                {
                    ...
                    {
                        strip_tbl[strip].usl7 = 1;
                        ref_vectors = new BytePtr(buf2, (buf1.Value * 2));
                        buf1.Offset++;
                        continue;
                    }
                }

                cur_frm_pos = cur + width * strip_tbl[strip].ypos + strip_tbl[strip].xpos;

                if ((blks_width = strip_tbl[strip].width) < 0)
                    blks_width += 3;
                blks_width >>= 2;
                blks_height = strip_tbl[strip].height;

                if (ref_vectors != BytePtr.Null)
                {
                    ref_frm_pos = @ref + ((sbyte)ref_vectors[0] + strip_tbl[strip].ypos) * width +
                        (sbyte)ref_vectors[1] + strip_tbl[strip].xpos;
                }
                else
                ...
                if (cmd == 2)
                {
                    ...
                    if (cmd == 0 || ref_vectors != BytePtr.Null)
                    {
                        for (lp1 = 0; lp1 < blks_width; lp1++)
                        {
                            for (i = 0, j = 0; i < blks_height; i++, j += width_tbl[1])
                                cur_frm_pos.WriteUInt32(j * 4, ref_frm_pos.ToUInt32(j * 4));
                            cur_frm_pos += 4;
                            ref_frm_pos += 4;
                        }
                    }
                ...
                else
                {
                    ...
                    if (k == 1 || k == 4)
                    {
                        ...
                        correction_type_sp[0] = new Ptr<ushort>(_corrector_type, (lv << 8) * 2);
                        correction_lp[0] = new Ptr<uint>(correction, (lv << 8) * 4);
                        lv = (hdr[j] >> 4) + fflags2;
                        correction_lp[1] = new Ptr<uint>(correction, (lv << 8) * 4);
                        correction_type_sp[1] = new Ptr<ushort>(_corrector_type, (lv << 8) * 2);
                    }
                    else
                    {
                        correctionloworder_lp[0] = correctionloworder_lp[1] = new Ptr<uint>(correctionloworder, (lv << 8) * 4);
                        correctionhighorder_lp[0] = correctionhighorder_lp[1] = new Ptr<uint>(correctionhighorder, (lv << 8) * 4);
                        correction_type_sp[0] = correction_type_sp[1] = new Ptr<ushort>(_corrector_type, (lv << 8) * 2);
                        correction_lp[0] = correction_lp[1] = new Ptr<uint>(correction, (lv << 8) * 4);
                    }
                    ...
                    switch (k)
                    {
                        ...
                        case 0:                    /********** CASE 0 **********/
                            for (; blks_height > 0; blks_height -= 4)
                            {
                                for (lp1 = 0; lp1 < blks_width; lp1++)
                                {
                                    for (lp2 = 0; lp2 < 4;)
                                    {
                                        k = buf1.Value; buf1.Offset++;

                                        cur_lp = new UIntAccess(cur_frm_pos.Data, cur_frm_pos.Offset + width_tbl[lp2] * 4);
                                        ref_lp = new UIntAccess(ref_frm_pos.Data, ref_frm_pos.Offset + width_tbl[lp2] * 4);

                                        switch (correction_type_sp[0][k])
                                        {
                                            case 0:
                                                cur_lp.Value = ((ref_lp.Value >> 1) + correction_lp[lp2 & 0x01][k]) << 1;
                                                lp2++;
                                                break;
                                            case 1:
                                                if (correction_type_sp[0][buf1.Value] != 1)
                                                {
                                                ...
                                                res = (ushort)(((ref_lp.Data.ToUInt16(ref_lp.Offset) >> 1) + correction_lp[lp2 & 0x01][buf1.Value]) << 1);
                                                cur_lp.Data.WriteUInt16(0, res);
                                                res = (ushort)(((ref_lp.Data.ToUInt16(ref_lp.Offset + 1) >> 1) + correction_lp[lp2 & 0x01][k]) << 1);
                                                cur_lp.Data.WriteUInt16(2, res);
                                                buf1.Offset++;
                                                lp2++;
                                                break;
                                            case 2:
                                                if (lp2 == 0)
                                                {
                                                    for (i = 0, j = 0; i < 2; i++, j += width_tbl[1])
                                                        cur_lp[j] = ref_lp.Data.ToUInt32(ref_lp.Offset + j);
                                                    lp2 += 2;
                                                }
                                                break;
                                            case 3:
                                            ...
                                            case 5:
                                            case 4:
                                                if (correction_type_sp[0][k] == 8)
                                                {
                                                    if (lp2 == 0)
                                                    {
                                                        RLE_V3_CHECK(ref buf1, rle_v1, rle_v2, rle_v3);
                                                        if (rle_v1 == 1 || ref_vectors != BytePtr.Null)
                                                        {
                                                            for (i = 0, j = 0; i < 4; i++, j += width_tbl[1])
                                                                cur_lp[j] = ref_lp.Data.ToUInt32(ref_lp.Offset + j);
                                                        }
                                                        RLE_V2_CHECK(ref buf1, rle_v2, rle_v3, lp2);
                                                        break;
                                                    }
                                                    else
                                                    {
                                                ...
                                                if (correction_type_sp[0][k] == 8 || correction_type_sp[0][k] == 5)
                                                {
                                                    LP2_CHECK(ref buf1, rle_v3, lp2);
                                                }
                                            ...
                                            case 9:
                                                ...
                                                LV1_CHECK(ref buf1, rle_v3, lv1, lp2);
                                                break;
                                        }
                                    }
                            ...
                            for (; blks_height > 0; blks_height -= 8)
                            {
                                for (lp1 = 0; lp1 < blks_width; lp1++)
                                {
                                    for (lp2 = 0; lp2 < 4;)
                                    {
                                        ...
                                        cur_lp = new UIntAccess(cur_frm_pos.Data, cur_frm_pos.Offset + width_tbl[lp2 * 2] * 4);
                                        ref_lp = new UIntAccess(cur_frm_pos.Data, cur_frm_pos.Offset + width_tbl[(lp2 * 2) - 1] * 4);

                                        switch (correction_type_sp[lp2 & 0x01][k])
                                        {
                                            case 0:
                                                cur_lp[width_tbl[1]] = ((ref_lp.Data.ToUInt32(ref_lp.Offset) >> 1) + correction_lp[lp2 & 0x01][k]) << 1;
                                                if (lp2 > 0 || flag1 == 0 || strip_tbl[strip].ypos != 0)
                                                    cur_lp[0] = ((cur_lp[-width_tbl[1]] >> 1) + (cur_lp[width_tbl[1]] >> 1)) & 0xFEFEFEFE;
                                                else
                                                    cur_lp[0] = ((ref_lp.Data.ToUInt32(ref_lp.Offset) >> 1) + correction_lp[lp2 & 0x01][k]) << 1;
                                                lp2++;
                                                break;

                                            case 1:
                                                res = (ushort)(((ref_lp.Data.ToUInt16(ref_lp.Offset) >> 1) + correction_lp[lp2 & 0x01][buf1.Value]) << 1);
                                                cur_lp.Data.WriteUInt16(width_tbl[2] * 2, res);
                                                res = (ushort)(((ref_lp.Data.ToUInt16(ref_lp.Offset + 2) >> 1) + correction_lp[lp2 & 0x01][k]) << 1);
                                                cur_lp.Data.WriteUInt16(width_tbl[2] * 2 + 2, res);
                                            ...
                                            case 2:
                                                ...
                                                {
                                                    for (i = 0, j = 0; i < 4; i++, j += width_tbl[1])
                                                        cur_lp[j] = ref_lp.Data.ToUInt32(ref_lp.Offset);
                                                    lp2 += 2;
                                                }
                                                break;
                                            ...
                                            case 5:
                                            case 4:
                                                if (correction_type_sp[lp2 & 0x01][k] == 8)
                                                {
                                                    if (lp2 == 0)
                                                    {
                                                        RLE_V3_CHECK(ref buf1, rle_v1, rle_v2, rle_v3);
                                                ...
                                                if (correction_type_sp[lp2 & 0x01][k] == 8 || correction_type_sp[lp2 & 0x01][k] == 5)
                                                {
                                                    LP2_CHECK(ref buf1, rle_v3, lp2);
                                                }
                                ...
                                for (; blks_height > 0; blks_height -= 8)
                                {
                                    for (lp1 = 0; lp1 < blks_width; lp1 += 2)
                                    {
                                        for (lp2 = 0; lp2 < 4;)
                                        {
                                            ...
                                            cur_lp = new UIntAccess(cur_frm_pos.Data, cur_frm_pos.Offset + width_tbl[lp2 * 2] * 4);
                                            ref_lp = new UIntAccess(cur_frm_pos.Data, cur_frm_pos.Offset + width_tbl[(lp2 * 2) - 1] * 4);
                                            lv1 = ref_lp.Data.ToInt32(ref_lp.Offset);
                                            lv2 = ref_lp.Data.ToInt32(ref_lp.Offset + 4);
                                            if (lp2 == 0 && flag1 != 0)
                                            {
#if SCUMM_BIG_ENDIAN
                                            										lv1 = lv1 & 0xFF00FF00;
                                            ...
                                            {
                                                case 0:
                                                    cur_lp[width_tbl[1]] = (uint)(((lv1 >> 1) + correctionloworder_lp[lp2 & 0x01][k]) << 1);
                                                    cur_lp[width_tbl[1] + 1] = (uint)(((lv2 >> 1) + correctionhighorder_lp[lp2 & 0x01][k]) << 1);
                                                    if (lp2 > 0 || strip_tbl[strip].ypos != 0 || flag1 == 0)
                                                    {
                                                        cur_lp[0] = ((cur_lp[-width_tbl[1]] >> 1) + (cur_lp[width_tbl[1]] >> 1)) & 0xFEFEFEFE;
                                                ...
                                                case 1:
                                                    if (correction_type_sp[lp2 & 0x01][buf1.Value] != 1)
                                                    {
                                                        //See the mode 0/1
                                                        //warning("Glitch");
                                                        return;
                                                    }
                                                    cur_lp[width_tbl[1]] = (uint)(((lv1 >> 1) + correctionloworder_lp[lp2 & 0x01][buf1.Value]) << 1);
                                                    cur_lp[width_tbl[1] + 1] = (uint)(((lv2 >> 1) + correctionloworder_lp[lp2 & 0x01][k]) << 1);
                                                    if (lp2 > 0 || strip_tbl[strip].ypos != 0 || flag1 == 0)
                                                    {
                                                        cur_lp[0] = ((cur_lp[-width_tbl[1]] >> 1) + (cur_lp[width_tbl[1]] >> 1)) & 0xFEFEFEFE;
                                                        cur_lp[1] = ((cur_lp[-width_tbl[1] + 1] >> 1) + (cur_lp[width_tbl[1] + 1] >> 1)) & 0xFEFEFEFE;
                                ...
                                for (; blks_height > 0; blks_height -= 8)
                                {
                                    for (lp1 = 0; lp1 < blks_width; lp1 += 2)
                                    {
                                        for (lp2 = 0; lp2 < 4;)
                                        {
                                            k = buf1.Value; buf1.Offset++;
                                            cur_lp = new UIntAccess(cur_frm_pos.Data, cur_frm_pos.Offset + width_tbl[lp2 * 2] * 4);
                                            ref_lp = new UIntAccess(ref_frm_pos.Data, ref_frm_pos.Offset + width_tbl[lp2 * 2] * 4);

                                            switch (correction_type_sp[lp2 & 0x01][k])
                                            {
                                                case 0:
                                                    lv1 = (int)correctionloworder_lp[lp2 & 0x01][k];
                                                    lv2 = (int)correctionhighorder_lp[lp2 & 0x01][k];
                                                    cur_lp[0] = (uint)(((ref_lp.Data.ToUInt32(ref_lp.Offset) >> 1) + lv1) << 1);
                                                    cur_lp[1] = (uint)(((ref_lp.Data.ToUInt32(ref_lp.Offset + 4) >> 1) + lv2) << 1);
                                                    cur_lp[width_tbl[1]] = (uint)(((ref_lp.Data.ToUInt32(ref_lp.Offset + width_tbl[1] * 4) >> 1) + lv1) << 1);
                                                    cur_lp[width_tbl[1] + 1] = (uint)(((ref_lp.Data.ToUInt32(ref_lp.Offset + width_tbl[1] * 4 + 4) >> 1) + lv2) << 1);
                                                    lp2++;
                                                    break;

                                                case 1:
                                                    lv1 = (int)correctionloworder_lp[lp2 & 0x01][buf1.Value]; buf1.Offset++;
                                                    lv2 = (int)correctionloworder_lp[lp2 & 0x01][k];
                                                    cur_lp[0] = (uint)(((ref_lp.Data.ToUInt32(ref_lp.Offset) >> 1) + lv1) << 1);
                                                    cur_lp[1] = (uint)(((ref_lp.Data.ToUInt32(ref_lp.Offset + 4) >> 1) + lv2) << 1);
                                                    cur_lp[width_tbl[1]] = (uint)(((ref_lp.Data.ToUInt32(ref_lp.Offset + width_tbl[1] * 4) >> 1) + lv1) << 1);
                                                    cur_lp[width_tbl[1] + 1] = (uint)(((ref_lp.Data.ToUInt32(ref_lp.Offset + width_tbl[1] * 4 + 4) >> 1) + lv2) << 1);
                                                ...
                                                case 2:
                                                    if (lp2 == 0)
                                                    {
                                                        for (i = 0, j = 0; i < 4; i++, j += width_tbl[1])
                                                        {
                                                            cur_lp[j] = ref_lp.Data.ToUInt32(ref_lp.Offset + j * 4);
                                                            cur_lp[j + 1] = ref_lp.Data.ToUInt32(ref_lp.Offset + j * 4 + 4);
                                                        }
                                                        lp2 += 2;
                                                    }
                                                    break;
                                                ...
                                                case 4:
                                                    if (correction_type_sp[lp2 & 0x01][k] == 8)
                                                    {
                                                        if (lp2 == 0)
                                                        {
                                                            ...
                                                            for (i = 0, j = 0; i < 8; i++, j += width_tbl[1])
                                                            {
                                                                cur_frm_pos.WriteUInt32(j * 4, ref_frm_pos.ToUInt32(ref_frm_pos.Offset + j * 4));
                                                                cur_frm_pos.WriteUInt32(j * 4 + 4, ref_frm_pos.ToUInt32(ref_frm_pos.Offset + j * 4 + 4));
                                                            }
                                                            RLE_V2_CHECK(ref buf1, rle_v2, rle_v3, lp2);
                                                            break;
                                                        }
                                                    ...
                                                    if (correction_type_sp[lp2 & 0x01][k] == 8 || correction_type_sp[lp2 & 0x01][k] == 5 || correction_type_sp[lp2 & 0x01][k] == 7)
                                                    {
                                                        LP2_CHECK(ref buf1, rle_v3, lp2);
                                                    }
                                                    for (i = 0, j = 0; i < 8 - (lp2 * 2); i++, j += width_tbl[1])
                                                ...
                                                case 9:
                                                    ...
                                                    for (i = 0, j = 0; i < 8; i++, j += width_tbl[1])
                                                    {
                                                        cur_frm_pos.WriteInt32(j * 4 + 4, lv);
                                                        cur_frm_pos.WriteInt32(j * 4, lv);
                                                    }
                                                    LV1_CHECK(ref buf1, rle_v3, lv1, lp2);
                                                    break;
                                            }
                        ...
                        default:
                            ...
                            Warning("Indeo3Decoder::decodeChunk: Unknown case {0}", k);
                            return;
                    }
                }
                ...
                for (; strip >= 0; strip--)
                {
                    if (strip_tbl[strip].split_flag != 0)
                    {
                        strip_tbl[strip].split_flag = 0;
                        strip_tbl[strip].usl7 = strip_tbl[strip - 1].usl7;

                        if (strip_tbl[strip].split_direction != 0)
                        {
                            strip_tbl[strip].xpos += strip_tbl[strip].width;
                            strip_tbl[strip].width = strip_tbl[strip - 1].width - strip_tbl[strip].width;
                            if (region_160_width <= strip_tbl[strip].xpos && width < strip_tbl[strip].width + strip_tbl[strip].xpos)
                                strip_tbl[strip].width = width - strip_tbl[strip].xpos;
                        }
                        else
                        {
                            strip_tbl[strip].ypos += strip_tbl[strip].height;
                            strip_tbl[strip].height = strip_tbl[strip - 1].height - strip_tbl[strip].height;
                        }
                        break;
                    }
                }
        ...
        private bool IsIndeo3(Stream stream)
        {
            ...
            {
                uint id0 = br.ReadUInt32();
                uint id1 = br.ReadUInt32();
                uint id2 = br.ReadUInt32();
                uint id3 = br.ReadUInt32();
                ...
                if ((id0 ^ id1 ^ id2 ^ id3) != ScummHelper.MakeTag('F', 'R', 'M', 'H'))
                    return false;
        ...
        private static readonly uint[] correction = {
    ...
    0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000
        ...
        private static readonly uint[] correctionloworder = {
    ...
    0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000
        ...
        private static readonly uint[] correctionhighorder = {
