		...
		void SaveOrLoad (Serializer serializer)
		{
			...
			var l_objStatesEntries = new[] {
				LoadAndSaveEntry.Create (reader => {
					...
					Array.Copy (objectOwnerTable, _resManager.ObjectOwnerTable, _numGlobalObjects);
				},
					writer => {
						writer.WriteBytes (_resManager.ObjectOwnerTable, _numGlobalObjects);
					}),
				LoadAndSaveEntry.Create (reader => {
					var objectStateTable = reader.ReadBytes (_numGlobalObjects);
					Array.Copy (objectStateTable, _resManager.ObjectStateTable, _numGlobalObjects);
				},
					writer => {
						writer.WriteBytes (_resManager.ObjectStateTable, _numGlobalObjects);
					})
			};
			...
			var l_globalObjStatesEntries = new[] {
				LoadAndSaveEntry.Create (reader => {
					Array.Copy (reader.ReadUInt32s (_numGlobalObjects), _resManager.ClassData, _numGlobalObjects);
				},
					writer => {
						writer.WriteUInt32s (_resManager.ClassData, _numGlobalObjects);
					}),
			};
