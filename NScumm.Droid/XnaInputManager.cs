...
namespace NScumm
{
    internal sealed class XnaInputManager : IInputManager
    {
        ...
        public XnaInputManager(Game game, Core.IO.IGameDescriptor gameDesc)
        {
            ...
            TouchPanel.EnabledGestures = GestureType.Hold | GestureType.Tap | GestureType.Pinch | GestureType.PinchComplete;
        ...
        public void UpdateInput(KeyboardState keyboard)
        {
            lock (_gate)
            {
                ...
                {
                    if (gestures[0].GestureType == GestureType.Pinch)
                    {
                    }
                    else if (gestures[0].GestureType == GestureType.PinchComplete)
                    {
                        _isMenuPressed = true;
                    }
                    else if (gestures[0].GestureType == GestureType.Tap)
                    {
