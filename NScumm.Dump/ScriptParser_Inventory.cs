...
namespace NScumm.Dump
{
	partial class ScriptParser
	{
		IEnumerable<Statement> FindInventory ()
		{
			...
			yield return SetResultExpression (index, new MethodInvocation ("FindInventory").AddArguments (x, y));
		}

		IEnumerable<Statement> GetInventoryCount ()
		{
			var index = GetResultIndexExpression ();
			yield return SetResultExpression (index, new MethodInvocation ("GetInventoryCount").AddArgument (GetVarOrDirectByte (OpCodeParameter.Param1)));
		}
	}
}
